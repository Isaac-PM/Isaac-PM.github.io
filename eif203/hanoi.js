...
var DISC1 = {x: canvas.width / 4 - 50, y: canvas.height - 10, width: 100, height: 10, rank: 5,
                   peg1X: canvas.width / 4 - 50, peg2X: canvas.width / 2 - 50, peg3X: 3 * canvas.width / 4 - 50, draw: function() {context.fillStyle = 'blue'; context.beginPath(); context.rect(this.x, this.y, this.width, this.height); context.fill();}};
var DISC2 = {x: canvas.width / 4 - 40, y: canvas.height - 20, width: 80, height: 10, rank: 4,
                   peg1X: canvas.width / 4 - 40, peg2X: canvas.width / 2 - 40, peg3X: 3 * canvas.width / 4 - 40,draw: function() {context.fillStyle = 'red'; context.beginPath(); context.rect(this.x, this.y, this.width, this.height); context.fill();}};
var DISC3 = {x: canvas.width / 4 - 30, y: canvas.height - 30, width: 60, height: 10, rank: 3,
                   peg1X: canvas.width / 4 - 30, peg2X: canvas.width / 2 - 30, peg3X: 3 * canvas.width / 4 - 30,draw: function() {context.fillStyle = 'green'; context.beginPath(); context.rect(this.x, this.y, this.width, this.height); context.fill();}};
var DISC4 = {x: canvas.width / 4 - 20, y: canvas.height - 40, width: 40, height: 10, rank: 2,
                    peg1X: canvas.width / 4 - 20, peg2X: canvas.width / 2 - 20, peg3X: 3 * canvas.width / 4 - 20,draw: function() {context.fillStyle = 'yellow'; context.beginPath(); context.rect(this.x, this.y, this.width, this.height); context.fill();}};
var DISC5 = {x: canvas.width / 4 - 10, y: canvas.height - 50, width: 20, height: 10, rank: 1,
                    peg1X: canvas.width / 4 - 10, peg2X: canvas.width / 2 - 10, peg3X: 3 * canvas.width / 4 - 10,draw: function() {context.fillStyle = 'white'; context.beginPath(); context.rect(this.x, this.y, this.width, this.height); context.fill();}};
var DISCLIST = [DISC1, DISC2, DISC3, DISC4, DISC5];
var PEG1 = [];
var PEG2 = [];
var PEG3 = [];

//Baseline canvas at start and on resetting number of discs.
function baseDraw() {
   STARTSELECTED = false;
   STARTPEG = undefined;
   TARGETPEG = undefined;
